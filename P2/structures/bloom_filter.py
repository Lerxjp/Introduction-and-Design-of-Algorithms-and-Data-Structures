...
class BloomFilter:
    ...
    *** A NOTE ON KEYS ***
    ...
    def __init__(self, max_keys: int) -> None:
        ...
        self.num_bits = self.get_num_bits(max_keys*100, self.fp_rate)
        self.num_hashes = self.get_num_hashes(self.num_bits, max_keys * 100)
        self._data.allocate(self.num_bits)
        self._empty = True
    ...
    def insert(self, key: Any) -> None:
        ...
        for i in range(self.num_hashes):
            index = (base_hash + i) % self.num_bits
            self._data.set_at(index)

        # After inserting, set the _empty flag to False
        self._empty = False
    ...
    def get_num_bits(self, size, fp):
        num_bits = -(size * math.log(fp)) / (math.log(2) ** 2)
        return int(num_bits)

    def get_num_hashes(self, bits, size):
        num_hashes = (bits / size) * math.log(2)
