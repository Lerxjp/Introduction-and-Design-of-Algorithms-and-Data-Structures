...
def maybe_maybe_maybe(database: list[str], query: list[str]) -> list[str]:
    ...
    # loads kmers into the filter
    for k in database:
        filter.insert(k)

    # checks if the kmers are in the filter
    for i in query:
...
def dora(graph: Graph, start: int, symbol_sequence: str,
    ...
    for i in symbol_sequence:
        if gene_table.__getitem__(i) == None:
            gene_table.insert_kv(i, 1)
        else:
            occurrence = gene_table.find(i) + 1
            gene_table.__setitem__(i, occurrence)

    for i in symbol_sequence:
        probs = gene_table.find(i)/gene_table.get_size()
        codebook.insert_kv(i, probs)
        coded_sequence.append(probs)
